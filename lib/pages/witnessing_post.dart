...
class WitnessingPost extends StatefulWidget {
  const WitnessingPost({super.key});

  @override
  State<WitnessingPost> createState() => _WitnessingPostState();
}

class _WitnessingPostState extends State<WitnessingPost> {
  // 컨트롤러
  ...
  Future<void> fetchUpload() async {
    try {
      ...
      await db.collection("witnessing").add(data).then((doc) {});
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
